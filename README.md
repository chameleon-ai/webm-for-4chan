...
| `--codec` | Video codec to use. The appropriate corresponding audio codec will be automatically selected. May be `libvpx-vp9`, `libx264`, `vp9_vaapi`, or [h264_nvenc](https://trac.ffmpeg.org/wiki/HWAccelIntro#NVENC). Default is libvpx-vp9. | `--codec libx264` |
...
If your hardware supports it, you can try [vp9](https://trac.ffmpeg.org/wiki/Hardware/VAAPI#Encode-only) and [h264](https://trac.ffmpeg.org/wiki/HWAccelIntro#NVENC) hardware acceleration  with `--codec vp9_vaapi` or `--codec h264_nvenc` respectively. Note that support for this is experimental since I can't personally get it to work on my machine, so I can't confirm that this option works properly. Your hardware needs to support `VAProfileVP9Profile0 : VAEntrypointEncSlice` through [VA-API](https://en.wikipedia.org/wiki/Video_Acceleration_API).\
